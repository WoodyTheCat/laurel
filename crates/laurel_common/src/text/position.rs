...
impl Ord for Position {
    fn cmp(&self, other: &Self) -> std::cmp::Ordering {
        match self.line.cmp(&other.line) {
            core::cmp::Ordering::Equal => self.character.cmp(&other.character),
            ord => ord,
        }
    }
}
...
impl PartialOrd for Position {
    fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> {
        match self.line.partial_cmp(&other.line) {
            Some(core::cmp::Ordering::Equal) => self.character.partial_cmp(&other.character),
            ord => ord,
        }
    }
}
...
impl Cursor {
    pub fn move_horizontally(&mut self, distance: isize) {
        if distance.is_positive() {
            ...
            self.0.character = self.0.character.saturating_sub(distance.unsigned_abs());
        }
    }
    ...
    pub fn move_vertically(&mut self, distance: isize) {
        if distance.is_positive() {
            ...
            self.0.line = self.0.line.saturating_sub(distance.unsigned_abs());
        }
    }
