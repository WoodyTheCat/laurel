    ...
    Renderer: iced::advanced::text::Renderer,
{
    pub fn new(
        ...
        Self {
            ...
            line_height: text::LineHeight::default().to_absolute(Pixels(font_size)).0,
            font_size,
            child,
            font,
            longest_line,
...
impl<'a, Message, Renderer> Widget<Message, LaurelTheme, Renderer>
    for TextboxContainer<'a, Message, Renderer>
where
    ...
    <Renderer as iced::advanced::text::Renderer>::Font: From<iced::Font>,
{
    ...
    fn overlay<'b>(
        ...
        if let Some(element) = self.floating_text.as_mut() {
            let point = self
                .textbox
                .get_window_point_from_position(self.textbox.float_position());
            let overlay = overlay::Element::new(Box::new(FloatingOverlay::new(
                element,
                &mut tree.children[1],
                layout.bounds().size(),
                point,
    ...
    fn size(&self) -> Size<Length> {
        Size::new(Length::Shrink, self.child.as_widget().size().height)
    }
    ...
    //                 let cursor = line_hit_test(
    //                     self.textbox.buffer().buffer(),
    //                     self.line_height,
    //                     text_width,
    //                     point,
    //                 );
...
fn line_hit_test(text: &Document, line_height: f32, glyph_width: f32, point: Point) -> Position {
    ...
    let line = text.get_line(line_num).unwrap_or_else(|| {
        line_num = text.len();
        text.last_line().unwrap()
    });
    let character = monospace_hit_test(
        line.as_str().unwrap_or(&line.to_string()),
        glyph_width,
        point,
    ) as usize;
